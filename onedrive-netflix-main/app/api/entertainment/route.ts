...
const graphEndpoint = `/beta/users/{user}/drive/root/items`
...
export async function POST(request: Request) {
  try {
    ...
    for (const folder in folders) {
      ...
      const access_token = await getAccessToken(account.email)

      const url = new URL(
        graphEndpoint.replace('{user}', account.email),
        graphDomain,
      )
      url.searchParams.append('$select', 'id,name,weburl,parentreference')
      url.searchParams.append('$filter', `name eq '${folder_name}'`)
      ...
      let childrenItemsRequestUrl = new URL(
        `/beta/users/${account.email}/drives/${driveId}/items/${itemId}/children`,
        graphDomain,
      )
